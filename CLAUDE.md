...
The runtime is a single-pass intent-first agent loop: **user text → intent → model action → tool call (optional) → observation → final answer**.
...
agent_runtime/
  ...
  model.py          # MockModel: maps intent to Action (no real LLM yet)
  loop.py           # AgentLoop: orchestrates the full loop with budget/cancellation guards
  tools.py          # ToolRegistry, ToolRunner, ToolSpec — mock_schema_context is the only wired tool
